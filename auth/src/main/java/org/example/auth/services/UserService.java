...
public class UserService {
    ...
    public int getTotalUsersCount() {
        String sql = "SELECT COUNT(*) FROM users";  // Requête pour compter tous les utilisateurs
        try (Connection connection = databaseService.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql);
             ResultSet resultSet = statement.executeQuery()) {
            if (resultSet.next()) {
                return resultSet.getInt(1);  // Retourne le nombre total d'utilisateurs
            }
        } catch (SQLException e) {
            System.err.println("Erreur lors de la récupération du nombre d'utilisateurs : " + e.getMessage());
        }
