...
public class UserViewReclamation {
    ...
    private void handleDelete(Reclamation reclamation) {
        if (showConfirmationDialog("Confirm Deletion", "Are you sure you want to delete this reclamation?")) {
            try {
    ...
    private void handleModify(Reclamation reclamation) {
        try {
            ...
            if (dialog.showAndWait().filter(response -> response == ButtonType.OK).isPresent()) {
                controller.updateReclamationFromFields();
                reclamationService.updateReclamation(reclamation);
                showAlert("Success", "Reclamation modified successfully.");
                refreshTable();
            }
        } catch (IOException | SQLException e) {
            e.printStackTrace();
            showAlert("Error", "Failed to modify the reclamation.");
        }
    }
    ...
    private void refreshTable() {
        try {
            ...
            if (!employees.isEmpty()) {
                ...
                // Fetch reclamations associated with the employee's ID
                List<Reclamation> reclamations = reclamationService.getReclamationsByUserId(employeeId);
                reclamationTable.getItems().setAll(reclamations);
            } else {
                showAlert("Error", "No employee found for the logged-in user.");
            }
        } catch (SQLException e) {
            e.printStackTrace();
